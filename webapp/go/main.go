...
func getNewItems(w http.ResponseWriter, r *http.Request) {
	...
	if itemID > 0 && createdAt > 0 {
		...
		err := dbx.Select(&items,
			"SELECT * FROM `items` WHERE `status` IN (?,?) AND (`created_at` < ? ) ORDER BY `created_at` DESC, `id` DESC LIMIT ?",
			ItemStatusOnSale,
			ItemStatusSoldOut,
	...
	sellerMap := selectSellers(sellerIDs)
...
func getUserItems(w http.ResponseWriter, r *http.Request) {
	...
	if itemID > 0 && createdAt > 0 {
		// paging
		err := dbx.Select(&items,
			"SELECT * FROM `items` WHERE `seller_id` = ? AND `status` IN (?,?,?) AND (`created_at` < ?) ORDER BY `created_at` DESC, `id` DESC LIMIT ?",
			userSimple.ID,
			ItemStatusOnSale,
			ItemStatusTrading,
			ItemStatusSoldOut,
...
func getTransactions(w http.ResponseWriter, r *http.Request) {
	...
	if itemID > 0 && createdAt > 0 {
		// paging
		err := dbx.Select(&items,
			"SELECT * FROM `items` WHERE (`seller_id` = ? OR `buyer_id` = ?) AND (`created_at` < ? ) ORDER BY `created_at` DESC, `id` DESC LIMIT ?",
			user.ID,
			user.ID,
			time.Unix(createdAt, 0),
	...
	buyerMap := selectSellers(buyerIDs)
	...
	for _, item := range items {
		...
		if transactionEvidence.ID > 0 {
			...
			case ShippingsStatusInitial:
				itemDetail.ShippingStatus = ShippingsStatusInitial
...
func selectSellers(sellerIDs []int64) map[int64]UserSimple {
	q, vs, _ := sqlx.In("SELECT * FROM `users` WHERE `id` IN (?) ", sellerIDs)
	us := []User{}
	dbx.Select(&us, q, vs...)
